...
import { ApiCredentialSchema, ApiPermissionResponse } from '@/modules/developer'
...
export default function DeveloperForm({ permissions }: Props) {
  ...
  const { state: { name, expiresAt }, errors, handleChange, validateForm, isFormValid } = useFormValidation({ name: '', expiresAt: '' }, ApiCredentialSchema)
          ...
          {permissions.map(permission => (
            <div key={permission.name} className="w-1/2 xl:w-1/4">
              <label className="flex items-center gap-2" htmlFor={`checkbox-${permission.name}`}>
                <input onChange={() => { }} className="w-4 h-4 form-input" id={`checkbox-${permission.name}`} type="checkbox" /> {permission.name}
              </label>
