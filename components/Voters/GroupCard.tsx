...
export default function GroupCard({ group, remove, handleRemove }: Props) {
  return (
    <div className="relative w-60 h-32 shadow rounded-lg bg-gray-200 hover:bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 p-4">
      <Link href={`/voters/groups/${group.id}`} className="w-max">
        <h3 className="mr-12 dark:hover:text-gray-100">{group.name} <i className="bi bi-box-arrow-in-up-right text-blue-500"></i></h3>
      </Link>
      <div className="absolute flex items-center gap-2 right-4 top-4">
        <Tooltip text="Edit" position="top">
          <span onClick={() => { }} className="text-blue-500 hover:text-blue-600 cursor-pointer">
